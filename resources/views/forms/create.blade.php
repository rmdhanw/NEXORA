<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('projects.show', $project->id) }}" class="p-2 bg-white rounded-full shadow-sm hover:bg-gray-50 transition">
            ...
            <div>
                <h2 class="font-black text-2xl text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-500 tracking-tight">
                    Tambah Master Form Baru - {{ $project->nama_project }}
                </h2>
                <p class="text-sm text-gray-500 mt-1">Buat form dinamis baru yang terisolasi di dalam project ini.</p>
            </div>
        </div>
    </x-slot>

    <div class="py-12" x-data="formBuilder()">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 rounded-3xl overflow-hidden">
                <form action="{{ route('forms.store') }}" method="POST" class="p-8">
                    @csrf
                    ...
                    <div class="mb-8 border-b pb-6">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" required class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm" placeholder="Contoh: Form Survey Kepuasan Pelanggan">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi (Opsional)</label>
                                <textarea name="description" rows="2" class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 transition shadow-sm" placeholder="Penjelasan singkat mengenai form ini..."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8 border-b pb-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-100 transition">
                                <input type="checkbox" name="has_photo" value="1" checked class="w-5 h-5 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500">
                                <span class="ml-3 font-semibold text-sm text-gray-800">Aktifkan Upload Foto Album (Cloudinary)</span>
                            </label>
                            <label class="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-100 transition">
                                <input type="checkbox" name="has_age_calc" value="1" checked class="w-5 h-5 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500">
                        ...
                        <div class="space-y-4">
                            <template x-for="(field, index) in fields" :key="index">
                                <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-200">
                                    <div class="flex-1">
                                        <input type="text" :name="`fields[${index}][name]`" x-model="field.name" placeholder="Nama Label (cth: Pekerjaan)" required class="block w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm">
                                    </div>
                                    <div class="w-48">
